...
export const useAuthStore = create<AuthStore>((set, get) => ({
  ...
  login: async (email: string, _password: string, isOAuth = false) => {
    ...
    // Mock Fallback
    if (isOAuth || (email === 'inspector.verma@cybercrime.gov.in' && _password === 'SecurePass@2026') || (email === 'auditor.gupta@cybercrime.gov.in' && _password === 'SecurePass@2026')) {
      const targetUser = {
        id: 'usr-001',
        email: email,
        username: email === 'auditor.gupta@cybercrime.gov.in' ? 'Auditor Gupta' : 'Inspector Verma',
      ...
      if (email === 'inspector.verma@cybercrime.gov.in' && !isOAuth) {
        set({ mfaPendingUser: targetUser, tempMfaToken: 'mock-temp-token-verma' });
        return true;
      }
