...
export const EvidencePage: React.FC = () => {
    ...
    const loadEvidence = async () => {
      try {
        const results = await Promise.all(targetCaseIds.map(async (caseId) => {
          const response = await fetch(`${API_BASE}/api/evidence/case/${caseId}`, {
  ...
  const handleVerify = async (id: string) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const response = await fetch(`${API_BASE}/api/evidence/verify/${id}`, {
        method: 'POST',
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
  ...
  const handleSealEvidence = async (id: string) => {
    const token = localStorage.getItem('token');
    try {
      const response = await fetch(`${API_BASE}/api/evidence/seal/${id}`, {
        method: 'POST',
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/evidence/upload/${targetCase}`, {
        method: 'POST',
        headers: token ? { Authorization: `Bearer ${token}` } : {},
        body: formData,
      });
      ...
      <div className="glass-card p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 w-full sm:w-auto">
          ...
          <select 
            ...
            className="input-field py-1.5 px-3 text-xs w-full sm:w-64 bg-dark-900 border-dark-700/50"
          >
