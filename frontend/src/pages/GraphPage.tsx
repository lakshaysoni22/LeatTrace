...
export const GraphPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const filteredNodes = mockGraphNodes.filter(n => {
      if (n.type === 'wallet' && !layers.wallets) return false;
      if (n.type === 'exchange' && !layers.entities) return false;
      if (n.type === 'contract' && !layers.entities) return false;
      return true;
    });

    const filteredEdges = mockGraphEdges.filter(e => {
  ...
  return (
    <div className="flex gap-4 animate-fade-in" style={{ height: 'calc(100vh - 120px)' }}>
      ...
      <div className="w-80 glass-card border-dark-700/50 flex flex-col justify-between overflow-hidden">
        ...
        {/* Footer Actions */}
        <div className="p-4 border-t border-dark-750/70">
          <button className="btn-ghost w-full py-2 flex items-center justify-center gap-2 text-xs font-semibold">
            <Download size={14} /> Export Graph JSON
          </button>
        </div>
      </div>
    </div>
