...
import { Bell, Search, Wifi, Shield, X, FolderPlus, FileUp, Sparkles, Keyboard } from 'lucide-react';
import { timeAgo, getSeverityColor } from '../../utils/helpers';

export const Header: React.FC = () => {
  const { sidebarOpen, currentPage, setPage, showShortcuts, setShowShortcuts } = useNavStore();
  ...
  return (
    <header className={`fixed top-0 right-0 h-16 bg-dark-900/80 backdrop-blur-xl border-b border-dark-700/50 
      flex items-center justify-between px-6 z-[100] transition-all duration-300
      ${sidebarOpen ? 'left-64' : 'left-[72px]'}`}
    >
      {/* Left */}
      <div className="flex items-center gap-4">
        <div>
          <h2 className="text-lg font-semibold text-white">{pageTitle[currentPage] || 'Dashboard'}</h2>
