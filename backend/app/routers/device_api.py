...
def trust_device(
    ...
    if existing:
        ...
        existing.trust_expires_at = datetime.datetime.utcnow() + datetime.timedelta(days=trust_days)
        db.commit()
    ...
    new_device = models.TrustedDevice(
        ...
        ip_address=ip,
        is_trusted=True,
        trust_expires_at=datetime.datetime.utcnow() + datetime.timedelta(days=trust_days),
    )
    db.add(new_device)
    db.commit()
